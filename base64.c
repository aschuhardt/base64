...
static void base64_init_decoder(void) {
  int i = 0;
...
static size_t base64_encsize(int size) { return 4 * ((size + 2) / 3); }

size_t base64_encode(char *dest, int size, unsigned char *src, int slen) {
  int dlen, i, j;
...
static int base64_decsize(char *src) {
  int slen, size, i;
